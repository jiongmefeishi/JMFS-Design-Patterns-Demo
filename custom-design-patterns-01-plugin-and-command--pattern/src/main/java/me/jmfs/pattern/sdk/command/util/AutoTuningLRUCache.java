...
public class AutoTuningLRUCache<K, V> extends BoundedConcurrentHashMap<K, V> {
    ...
    private int targetHitRateDiffValue = 2; //当前命中率与目标命中率差距多少时才做调整
    ...
    private void tuning() {
        ...
        if (dvalue > targetHitRateDiffValue) {
        ...
        } else if (dvalue < -targetHitRateDiffValue) {
    ...
    public String report() {
        return new StringBuilder("MaxCacheSize:").append(maxCacheSize).append(",MinCacheSize:").append(minCacheSize)
            ...
            .append(",TargetHitRateDvalue:").append(targetHitRateDiffValue).append(",autoTuningInterval:")
    ...
    public int getTargetHitRateDiffValue() {
        return targetHitRateDiffValue;
    }


    public void setTargetHitRateDiffValue(int targetHitRateDiffValue) {
        this.targetHitRateDiffValue = targetHitRateDiffValue;
    }
